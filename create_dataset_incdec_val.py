...
mask_dir_inc = './datasets/CoCoClutter/incdec/maskdir_val'
...
for counter in tqdm(range(0,nb_images), desc='image'):
    ...
    for i in range(len(anns)):
        ...
        if np.mean(salmap * minim) * (np.size(minim) / np.sum(minim)) > 0.7:
            continue
        if np.mean(salmap * minim) * (np.size(minim) / np.sum(minim)) < 0.2:
            continue
    ...
    if len(maski)>1:
        [Image.fromarray(maski[pick]).save(os.path.join(mask_dir_inc, namei[pick])) for pick in range(len(namei))]
        counter +=1
